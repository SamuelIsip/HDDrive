...
function createList(doc: any, div_list: Element) {
  ...
  doc.docs.forEach((nameDoc: IFolder) => {
    //Body
...
function getPath() {
  ...
  for (let i = 2; i < li_list.length; i++) {
    rutaHeader =
      rutaHeader + (<HTMLElement>li_list[i].children[0]).innerText + "/";
  }
...
function updateLinkHead(nameDir: string) {
  ...
  for (let i = lis.length - 1; i > 1; i--) {
    if (
      (<HTMLElement>docs_header.children[i].firstElementChild).innerText ==
      nameDir
    )
      break;
...
function calculateTotalStorage() {
  $.ajax({
    ...
    success: (responseText) => {
      var number_size: number = 0;
      responseText.storage.forEach((t: any) => {
